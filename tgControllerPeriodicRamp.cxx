...
// TODO: consider re-implementing tgControllerPeriodic in terms of
// tgControllerPeriodicRamp
tgControllerPeriodicRamp::tgControllerPeriodicRamp(tgPerception* perception, 
                                                   tgController* controller,
                                                   tgPerceptReal distance,
                                                   tgReal const nearDistance,
                                                   tgReal const farDistance,
                                                   int const minPeriod,
                                                   int const maxPeriod) :
...
tgControllerPeriodicRamp::~tgControllerPeriodicRamp()
{
}

// TODO: inline these
void tgControllerPeriodicRamp::setMinPeriod(int const minPeriod)
{
   this->minPeriod = minPeriod;
}

int tgControllerPeriodicRamp::getMinPeriod() const
{
   return minPeriod;
}

void tgControllerPeriodicRamp::setMaxPeriod(int const maxPeriod)
{
   this->maxPeriod = maxPeriod;
}

int tgControllerPeriodicRamp::getMaxPeriod() const
{
   return maxPeriod;
}

void tgControllerPeriodicRamp::setController(tgController* controller)
{
   this->controller = controller;
}

tgController* tgControllerPeriodicRamp::getController() const
{
   return controller;
}

tgPerceptReal tgControllerPeriodicRamp::getDistance() const
{
   return distance;
}

tgReal tgControllerPeriodicRamp::getNearDistance() const
{
   return nearDistance;
}

tgReal tgControllerPeriodicRamp::getFarDistance() const
{
   return farDistance;
}

void tgControllerPeriodicRamp::setDistance(tgPerceptReal const distance)
{
   this->distance = distance;
}

void tgControllerPeriodicRamp::setNearDistance(tgReal const nearDistance)
{
   this->nearDistance = nearDistance;
}

void tgControllerPeriodicRamp::setFarDistance(tgReal const farDistance)
{
   this->farDistance = farDistance;
}

void tgControllerPeriodicRamp::calcAction()
{
   int const time(perception->getTime());

   tgReal const periodUnclamped((tgReal(maxPeriod)/farDistance) * (perception->*(distance))());
   int const period(int(tgMath::clamp(periodUnclamped, minPeriod, maxPeriod)));

#if 0
   tgReal const d((perception->*(distance))());

   tgReal const slope(tgReal(maxPeriod - minPeriod)/(nearDistance - farDistance));
   tgReal const intercept(tgReal(maxPeriod) + nearDistance * slope);
   int const period(tgMath::clamp(tgMath::round(d * slope + intercept), minPeriod, maxPeriod));
#endif
