...
tgCharacterRenderer::tgCharacterRenderer(tgGameState* gs) :
   tgObstacleCircleRenderer(gs)
{
}

tgCharacterRenderer::~tgCharacterRenderer()
{
}

void tgCharacterRenderer::render(tgObject const* o)
{
   tgCharacter const* c(static_cast<tgCharacter const*>(o));

   if (c->getIsTagged()) { flashingOn(); }
   else { flashingOff(); }

   tgObstacleCircleRenderer::render(o);

   // return;

   tgRealVec start(o->getPosition());
   tgRealVec v(o->getOrientation());
   v.scale(c->getRadius());
   start.add(v);
   v = o->getOrientation();
   v.scale(1.5 * c->getRadius() * (c->getSpeed()/c->getMaxSpeed()));
   tgGui::drawArrow(start, v);
}
