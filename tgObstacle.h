...
namespace tagGame
{
   class tgObstacleCircle;
   class tgObstacleSide;

   /// Common base class for all objects that can act as obstacles.
   class tgObstacle : public tgObject
   {
   public:
      tgObstacle(tgGameState* gs);

      virtual ostream& output(ostream& out) const = 0;

      bool touching(tgObstacle const& o) const;
      bool colliding(tgObstacle const& o) const;

      /// Calculate the nearest intersection point of a line in the
      /// direction v from the point p.
      virtual tgRealVec nearestIntersection(tgRealVec const& p, tgRealVec const& v) const = 0;

      /// Return a vector that is normal to o, with respect to this.
      virtual tgRealVec normalTo(tgObstacle const& o) const = 0;
      ...
      virtual tgReal distanceTo(tgObstacle const& o) const = 0;
      virtual tgReal distanceTo(tgObstacleCircle const& c) const = 0;
      virtual tgReal distanceTo(tgObstacleSide const& s) const = 0;

      tgReal distanceSquaredTo(tgObstacle const& o) const;
   private:
   };

   ostream& operator<<(ostream& out, tgObstacle const& o);
