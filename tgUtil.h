...
namespace tagGame
{
   ...
   public:
      ...
      static void warn(string const& message);

      /// Print the error message and exit.
      static void error(string const& message);

      /// Called by the TG_ASSERT macros.
      static void error(string const& errorStr, int const line, string const& filename, string const& message = "");

      /// Iff condition is false, print the warning message.
      static void warn(bool const condition, string const& message);
...
#if defined(DEBUG)
   #define TG_ASSERT(c) ((c) ? (void)0 : tagGame::tgUtil::error(#c, __LINE__, __FILE__))
   #define TG_ASSERT_MSG(c, s) ((c) ? (void)0 : tagGame::tgUtil::error(#c, __LINE__, __FILE__, s))
#else
   #define TG_ASSERT(c)
