...
void tgUtil::warn(string const& message)
{
   cerr << message << endl;
}

void tgUtil::error(string const& message)
{
   warn(message);
   exit(0);
}

void tgUtil::error(string const& errorStr, int const line, string const& filename, string const& message)
{
   cerr << "assert failed: " << errorStr << " (Line: " << line << ", file: " << filename << ")" << endl;
   error(message);
}

void tgUtil::warn(bool const condition, string const& message)
{
   if (!condition)
   {
      warn(message);
