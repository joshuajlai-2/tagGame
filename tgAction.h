...
namespace tagGame
{
   ...
   public:
      tgAction();
      tgAction(tgAction const& a);

      tgAction& operator=(tgAction const& a);

      inline void setDesiredDirection(tgRealVec const& direction);
      inline tgRealVec const& getDesiredDirection() const;
      inline void setDesiredSpeed(tgReal const speed);
      inline tgReal getDesiredSpeed() const;
   ...
   ostream& operator<<(ostream& out, tgAction const& a);

   void tgAction::setDesiredDirection(tgRealVec const& direction)
   {
      this->direction = direction;
   }
   
   tgRealVec const& tgAction::getDesiredDirection() const
   {
      return direction;
   }
   
   void tgAction::setDesiredSpeed(tgReal const speed)
   {
      this->speed = speed;
   }
   
   tgReal tgAction::getDesiredSpeed() const
   {
      return speed;
   }
}
