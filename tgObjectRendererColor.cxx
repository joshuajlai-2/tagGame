...
#include "tgObjectRendererColor.h"
#include "tgObject.h"
...
tgObjectRendererColor::tgObjectRendererColor(tgGameState* gs) :
   tgObjectRenderer(gs),
   color(tgGui::colorDim),
   colorAlternative(tgGui::colorDim),
   colorFlash(tgGui::colorDim),
   flash(false),
   alternative(false)
{
}

tgObjectRendererColor::~tgObjectRendererColor()
{
}

void tgObjectRendererColor::render(tgObject const* o)
{
   if (alternative) { tgGui::setColor(colorAlternative); }
   else { tgGui::setColor(color); }

   if (flash && gs->getTimer()->gameTime() % 2000 < 1000)
   {
      tgGui::setColor(colorFlash);
   }
}
