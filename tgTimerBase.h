...
#ifndef TG_TIMER_BASE_H
#define TG_TIMER_BASE_H

#include "tgCharacter.h"

namespace tagGame
{
   /// The tgTimerBase class.
   template <typename T>
   class tgTimerBase
   {
   public:
      ...
      /// Game time.
      /// Measures the time (in 1/ticksPerSec seconds) in the game.
      /// Note: unlike wall clock time (see below), game time halts if
      /// the game is paused and then re-starts when the game is unpaused.
      int gameTime() const;

      /// Wall clock time.
      /// Measures the time (in 1/ticksPerSec seconds) since the game started.
   ...
   private:
      ...
      tgTimerBase(tgTimerBase const& timer);

      /// Purposefully not implemented.
      tgTimerBase& operator=(tgTimerBase const& timer);
   ...
   template <typename T>
   void tgTimerBase<T>::stop()
   {
      if (isRunning)
      {
         baseTime = baseTime + (time() - startTime);
         isRunning = false;
      }
   }
   
   template <typename T>
   bool tgTimerBase<T>::isStopped() const
   {
      return !isRunning;
   }
   
   template <typename T>
   int tgTimerBase<T>::gameTime() const
   {
      if (isRunning)
      {
         return baseTime + (time() - startTime);
      }
      else
      {
         return baseTime;
   ...
   // The time method is implemented using template polymorphism because it is
   // platform dependent.
   template <typename T>
   int tgTimerBase<T>::time() const
   {
      return static_cast<T const*>(this)->time();
   }
}
