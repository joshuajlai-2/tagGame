...
namespace tagGame
{
   ...
   typedef double tgReal;
   const tgReal Inf = HUGE_VAL;
   ...
   public:
      ...
      inline static bool isAlmostEq(tgReal const x, tgReal const y);

      /// True iff x is within eps of zero.
      inline static bool isAlmostZero(tgReal const x);

      /// Round x to the nearest integer value.
      inline static int round(tgReal const x);

      /// Return the sign of x.
      inline static int sgn(tgReal const x);

      /// Return t in degrees.
      inline static tgReal radToDeg(tgReal const t);

      /// Return t in radians.
      inline static tgReal degToRad(tgReal const t);

      /// Clamp x to the closed range [lower, upper].
      inline static tgReal clamp(tgReal const x, tgReal lower, tgReal upper);
      ...
      inline static tgReal angleClamp(tgReal const t);

      /// Uniformly pick a tgReal in the range [0, 1].
      inline static tgReal uniform01();

      /// Uniformly pick an integer in the range [0, n).
      inline static int uniform(int const n);

      /// Pick a tgReal according to the normal distribution with mean and standard
      /// deviation as provided.
      static tgReal normal(tgReal const mean = tgReal(0), tgReal const std = tgReal(1));
      ...
      inline static tgReal normalAngle(tgReal const mean = tgReal(0), tgReal const std = tgReal(70));
   ...
   bool tgMath::isAlmostEq(tgReal const x, tgReal const y)
   {
      tgReal const eps(0.00001);
   
      return (fabs(x - y) <= eps) || (x == Inf && y == Inf);
   }
   
   bool tgMath::isAlmostZero(tgReal const x)
   {
      return isAlmostEq(x, tgReal(0));
   }
   
   int tgMath::round(tgReal const x)
   {
      return int(x + 0.5);
   }
   
   int tgMath::sgn(tgReal const x)
   {
      return x < 0 ? -1 : 1;
   }
   
   tgReal tgMath::radToDeg(tgReal const x)
   {
      return x * tgReal(180) / M_PI;
   }
   
   tgReal tgMath::degToRad(tgReal const x)
   {
      return x * M_PI / tgReal(180);
   }
   
   tgReal tgMath::uniform01()
   {
      // TODO: use a better random number generator.
      return tgReal(rand())/tgReal(RAND_MAX);
   }
   
   int tgMath::uniform(int const n)
   {
      TG_ASSERT_MSG(n <= RAND_MAX, "n: " + tgUtil::itos(n) + " must be <= RAND_MAX: " + tgUtil::itos(RAND_MAX));
      // TODO: use a better random number generator.
      return rand() % n;
   }
   
   tgReal tgMath::clamp(tgReal x, tgReal lower, tgReal upper)
   {
   ...
   tgReal tgMath::angleClamp(tgReal const t)
   {
      ...
      tgReal const x(t == tgReal(-360) ? t : tgReal(sgn(t)) * fmod(fabs(t), tgReal(360)));
      TG_ASSERT(tgReal(-360) <= x && x < tgReal(360));
      return x < tgReal(-180) ? x + tgReal(360) : (x >= tgReal(180) ? x - tgReal(360) : x);
   }
   
   tgReal tgMath::normalAngle(tgReal const mean, tgReal const std)
   {
      return angleClamp(normal(mean, std));
   }
/*
   tgRealVec tgMath::findMedian(tgRealVec theList, tgReal index1, tgReal index2, tgReal curPos)
   {
      if(index1 == index2)
	  {
		  return curPos;
