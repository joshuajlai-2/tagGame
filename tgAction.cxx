...
tgAction::tgAction() :
   direction(tgUtil2D::dim),
   speed(tgReal(0))
{
   // Head in direction (1, 0) to start with
   direction[0] = tgReal(1);
}

tgAction::tgAction(tgAction const& a)
{
   *this = a;
}

tgAction& tgAction::operator=(tgAction const& a)
{
   if (this != &a)
   {
...
ostream& tgAction::output(ostream& out) const
{
   out << "(" << direction << ") * " << speed;
   return out;
}

ostream& tagGame::operator<<(ostream& out, tgAction const& a)
{
   return a.output(out);
}
