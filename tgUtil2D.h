...
namespace tagGame
{
   ...
   public:
      ...
      static tgRealMatrix perpendicularTo(tgRealVec const& v);
      /// Return the vector perpendicular to v that is closest in direction to d.
      static tgRealVec perpendicularTo(tgRealVec const& v, tgRealVec const& d);
      static tgRealVec dir(tgReal const t);
      static tgRealVec uniformDir();
      static tgRealVec normalDir(tgReal const mean, tgReal const std);
      static tgReal angle(tgRealVec const& v);
      static tgRealMatrix rotationMatrix(tgReal const t);
      static tgRealVec randomPosition(tgRealVec const& worldDim);
      /// Computes points on circle and on tangent line of p to
      /// circle.
      static tgRealMatrix tagentLinePts(tgRealVec const& p, tgRealVec const& center, tgReal const radius);
      /// Computes points on circle that intersects with the line
      /// perpendicular to the line from p to center.
      static tgRealMatrix perpendicularLinePts(tgRealVec const& p, tgRealVec const& center, tgReal const radius);
   };
}
