...
#ifndef TG_CONTROLLER_PC_H
#define TG_CONTROLLER_PC_H
...
namespace tagGame
{
   ...
   template<class T>
   class tgControllerPC : public tgController
   {
   public:
      tgControllerPC(tgPerception* perception);
      virtual ~tgControllerPC();
   ...
   template<class T>
   tgControllerPC<T>::tgControllerPC(tgPerception* perception) :
      tgController(perception)
   {
   }
   
   template<class T>
   tgControllerPC<T>::~tgControllerPC()
   {
   }
   
   template<class T>
   bool tgControllerPC<T>::init()
   {
      return joystick.init();
   }
   
   template<class T>
   void tgControllerPC<T>::calcAction()
   {
      ...
      {
         // direction unchanged
         action.setDesiredSpeed(tgReal(0));
      }
      else
      {
