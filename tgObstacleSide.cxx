...
tgObstacleSide::tgObstacleSide(tgGameState* gs) :
   tgObstacle(gs),
   normal(tgUtil2D::dim),
   distance(tgReal(0))
{
...
ostream& tgObstacleSide::output(ostream& out) const
{
...
tgRealVec const& tgObstacleSide::getNormal() const
{
   return normal;
}

// TODO: make this inline
tgReal tgObstacleSide::getDistance() const
{
   return distance;
}

tgRealVec const& tgObstacleSide::getBegin() const
{
   return begin;
}

tgRealVec const& tgObstacleSide::getEnd() const
{
   return end;
}

// TODO: make this inline
void tgObstacleSide::setNormal(tgRealVec const& normal)
{
...
// TODO: make this inline
void tgObstacleSide::setDistance(tgReal const distance)
{
   this->distance = distance;
}

void tgObstacleSide::setBegin(tgRealVec const& begin)
{
   this->begin = begin;
}

void tgObstacleSide::setEnd(tgRealVec const& end)
{
   this->end = end;
}

tgRealVec tgObstacleSide::nearestIntersection(tgRealVec const& p, tgRealVec const& v) const
{
   tgRealVec u(end.relativeTo(begin));
   tgRealVec r(tgUtil2D::dim);

   // Following code assumes 2D.
   TG_ASSERT(2 == tgUtil2D::dim);

   // TODO: turn collision detection for sides back on by deleting next 2 lines
   r.set(Inf);
   return r;

   tgReal const d(v[1] * u[0] - v[0] * u[1]);
   // The two lines are parallel.
   if (tgMath::isAlmostZero(d))
   {
      r.set(Inf);
      return r;
   }

   tgReal const s((v[0] * (begin[1] - p[1]) - v[1] * (begin[0] - p[0])) / d);
   ...
   tgReal const t((u[0] * (begin[1] - p[1]) - u[1] * (begin[0] - p[0])) / d);
...
tgRealVec tgObstacleSide::normalTo(tgObstacle const& o) const
{
#if 0
   tgRealVec v(end.relativeTo(begin));
   v.normalize();
   if (normal != tgUtil2D::perpendicularTo(v))
   {
      cerr << begin << "; " << end << ";" << normal << "; " << tgUtil2D::perpendicularTo(v) << endl;
      TG_ASSERT(false);
   }
#endif
   return normal;
...
tgReal tgObstacleSide::distanceTo(tgObstacle const& o) const
{
   return o.distanceTo(*this);
}

tgReal tgObstacleSide::distanceTo(tgObstacleCircle const& c) const
{
   return Inf; // TODO: delete this line to turn collisions with sides back on
   tgReal const d(normal.dot(c.getPosition()) - distance);

   return d < 0 ? d + c.getRadius() : d - c.getRadius();
}

tgReal tgObstacleSide::distanceTo(tgObstacleSide const& s) const
{
   tgUtil::error("tgObstacleSide::distanceTo not implemented for side obstacles");
