...
#ifndef TG_OBJECT_RENDERER_COLOR_H
#define TG_OBJECT_RENDERER_COLOR_H

#include "tgObjectRenderer.h"
#include "tgVec.h"
...
namespace tagGame
{
   ...
   /// A class to handle object's colors.
   class tgObjectRendererColor : public tgObjectRenderer
   {
   public:
      tgObjectRendererColor(tgGameState* gs);
      virtual ~tgObjectRendererColor();

      virtual void render(tgObject const* o);

      inline tgRealVec const& getColor() const;
      inline tgRealVec const& getColorAlternative() const;
      inline tgRealVec const& getColorFlash() const;

      inline void setColor(tgRealVec const& color);
      inline void setColorAlternative(tgRealVec const& colorAlternative);
      inline void setColorFlash(tgRealVec const& colorFlash);
   ...
   private:
      tgRealVec color;
      tgRealVec colorAlternative;
      tgRealVec colorFlash;

      bool flash;
      bool alternative;
   };

   tgRealVec const& tgObjectRendererColor::getColor() const
   {
      return color;
   }
   
   tgRealVec const& tgObjectRendererColor::getColorAlternative() const
   {
      return colorAlternative;
   }
   
   tgRealVec const& tgObjectRendererColor::getColorFlash() const
   {
      return colorFlash;
   }
   
   void tgObjectRendererColor::setColor(tgRealVec const& color)
   {
      this->color = color;
   }
   
   void tgObjectRendererColor::setColorAlternative(tgRealVec const& colorAlternative)
   {
      this->colorAlternative = colorAlternative;
   }
   
   void tgObjectRendererColor::setColorFlash(tgRealVec const& colorFlash)
   {
      this->colorFlash = colorFlash;
   }
   
   void tgObjectRendererColor::flashingOn()
   {
      flash = true;
   }
   
   void tgObjectRendererColor::flashingOff()
   {
      flash = false;
   }
   
   void tgObjectRendererColor::alternativeOn()
   {
      alternative = true;
   }
   
   void tgObjectRendererColor::alternativeOff()
   {
      alternative = false;
   }
}
