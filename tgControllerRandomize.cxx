...
tgControllerRandomize::tgControllerRandomize(tgPerception* perception,
                                             tgController* controller,
                                             tgPerceptReal const distance,
...
tgControllerRandomize::~tgControllerRandomize()
{
}

// TODO: most const pointers, more consistency in using Ptr suffix

tgController const* tgControllerRandomize::getController() const
{
   return controller;
}

tgPerceptReal tgControllerRandomize::getDistance() const
{
   return distance;
}

tgReal tgControllerRandomize::getNearDistance() const
{
   return nearDistance;
}

tgReal tgControllerRandomize::getFarDistance() const
{
   return farDistance;
}

void tgControllerRandomize::setController(tgController* controller)
{
   this->controller = controller;
}

void tgControllerRandomize::setDistance(tgPerceptReal const distance)
{
   this->distance = distance;
}

void tgControllerRandomize::setNearDistance(tgReal const nearDistance)
{
   this->nearDistance = nearDistance;
}

void tgControllerRandomize::setFarDistance(tgReal const farDistance)
{
   this->farDistance = farDistance;
}

void tgControllerRandomize::calcAction()
{
   controller->calcAction();
   tgAction const a(controller->getAction());

   tgReal const d((perception->*(distance))());

   // Compute the distance as a fraction of "farDistance"
   tgReal const dFrac(std::min(tgReal(1), d/farDistance));

   tgReal const angle(tgUtil2D::angle(a.getDesiredDirection()) + dFrac * tgReal(tgMath::uniform(360) - 180));
   action.setDesiredDirection(tgUtil2D::dir(angle));
   ...
   tgReal const stdMax(130);
   tgReal const std(std::min(stdMax, stdMax * d/farDistance));

   v = tgUtil2D::normalDir(tgUtil2D::angle(v), std);
#endif
