...
#ifndef TG_OBSTACLE_SIDE_H
#define TG_OBSTACLE_SIDE_H

#include "tgObstacle.h"

namespace tagGame
{
   /// The sides of the world are a special kind of obstacle
   class tgObstacleSide : public tgObstacle
   {
   public:
      // TODO: more arguments in constructor, ditto for circle obstacles
      tgObstacleSide(tgGameState* gs);

      virtual ostream& output(ostream& out) const;

      void setNormal(tgRealVec const& normal);
      void setDistance(tgReal const distance);
      tgRealVec const& getNormal() const;
      tgReal getDistance() const;
      tgRealVec const& getBegin() const;
      tgRealVec const& getEnd() const;

      void setBegin(tgRealVec const& begin);
      void setEnd(tgRealVec const& end);

      virtual tgRealVec nearestIntersection(tgRealVec const& p, tgRealVec const& v) const;

      virtual tgRealVec normalTo(tgObstacle const& o) const;

      virtual tgReal distanceTo(tgObstacle const& o) const;
      virtual tgReal distanceTo(tgObstacleCircle const& c) const;
      virtual tgReal distanceTo(tgObstacleSide const& s) const;
