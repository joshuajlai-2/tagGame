...
tgControllerEvade::tgControllerEvade(tgPerception* perception) :
   tgController(perception)
{
}

tgControllerEvade::~tgControllerEvade()
{
}
...
void tgControllerEvade::calcAction()
{
   tgRealVec v(perception->myPosition().relativeTo(perception->taggedPosition()));

   // Following assert would fail if I am tagged character, i.e. can't evade myself!
   // TODO: necessary?
   TG_ASSERT(!tgMath::isAlmostZero(v.length()));

   // TODO: consider predicating on v.length() e.g. inversely proportional so
   // that speed increases as tagged character gets closer
   // std::max(tgReal(0), std::min(tgReal(1), tgReal(1) - (tgReal(0.25)*tagDist)/tagFar));
   action.setDesiredSpeed(tgReal(.5));
