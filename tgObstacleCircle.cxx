...
tgObstacleCircle::tgObstacleCircle(tgGameState* gs) :
   tgObstacle(gs),
   radius(10.0)
{
}

ostream& tgObstacleCircle::output(ostream& out) const
{
...
tgReal tgObstacleCircle::distanceTo(tgObstacle const& o) const
{
   return o.distanceTo(*this);
}

tgReal tgObstacleCircle::distanceTo(tgObstacleCircle const& c) const
{
   return (getPosition().relativeTo(c.getPosition())).length() - getRadius() - c.getRadius();
}

tgReal tgObstacleCircle::distanceTo(tgObstacleSide const& s) const
{
...
tgRealVec tgObstacleCircle::nearestIntersection(tgRealVec const& p, tgRealVec const& v) const
{
   TG_ASSERT(tgMath::isAlmostEq(v.length(), 1));

   tgRealVec const rp(p.relativeTo(getPosition()));
   tgReal const k0(rp.squaredLength() - getRadius() * getRadius());
   tgReal const k1(v.dot(rp));

   vector<tgReal> roots;
   tgReal const k(k1 * k1 - k0);
   if (tgMath::isAlmostZero(k))
   {
      roots.push_back(-k1);
   }
   else if (0 < k)
   {
      tgReal const kSqrt(sqrt(k));
      roots.push_back(-k1 - kSqrt);
      roots.push_back(-k1 + kSqrt);
      TG_ASSERT(roots[0] < roots[1]);
   }

   tgRealVec q(tgUtil2D::dim);
   ...
   {
      if (tgMath::isAlmostZero(roots[i]) || 0 < roots[i])
      {
         q = v;
         q.scale(roots[i]);
         q.add(p);
...
tgRealVec tgObstacleCircle::normalTo(tgObstacle const& o) const
{
   // TODO: redesign to eliminate the need for the dynamic_cast
   if (dynamic_cast<tgObstacleCircle const*>(&o))
   {
      return (o.getPosition().relativeTo(getPosition())).normalize();
   }
   return o.normalTo(static_cast<tgObstacle const&>(*this)).scale(-1);
}
