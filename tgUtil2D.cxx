...
tgRealMatrix tgUtil2D::perpendicularTo(tgRealVec const& v)
{
...
tgRealVec tgUtil2D::perpendicularTo(tgRealVec const& v, tgRealVec const& d)
{
   TG_ASSERT(tgMath::isAlmostEq(1,v.length()));
   TG_ASSERT(tgMath::isAlmostEq(1,d.length()));

   tgRealMatrix m(perpendicularTo(v));
   tgReal const cosA(d.dot(m.getRow(0)));
   // tgReal const cosB(d.dot(m.getRow(1)));

   if (tgMath::isAlmostZero(cosA)) { return m.getRow(0); }
...
tgRealVec tgUtil2D::dir(tgReal const t)
{
   tgRealVec v(dim);
   tgReal const tr(tgMath::degToRad(t));
   v[0] = cos(tr);
   v[1] = sin(tr);
...
tgRealVec tgUtil2D::uniformDir()
{
   return dir(tgMath::uniform01() * tgReal(360) - tgReal(180));
}

tgRealVec tgUtil2D::normalDir(tgReal const mean, tgReal const std)
{
   return dir(tgMath::normalAngle(mean, std));
}

tgReal tgUtil2D::angle(tgRealVec const& v)
{
   TG_ASSERT(dim == v.size());

   tgRealVec u(v);
   u.normalize();
   return tgMath::radToDeg(atan2(u[1], u[0]));
}

tgRealMatrix tgUtil2D::rotationMatrix(tgReal const t)
{
   tgRealMatrix m(dim, dim);
...
tgRealVec tgUtil2D::randomPosition(tgRealVec const& worldDim)
{
   tgRealVec p(dim);
...
tgRealMatrix tgUtil2D::tagentLinePts(tgRealVec const& p, tgRealVec const& center, tgReal const radius)
{
   tgRealVec v(center.relativeTo(p));
   tgReal const d(v.length());

   TG_ASSERT_MSG(d < radius, "tangentLinePts: point is inside circle");

   tgReal const alpha(atan2(v[1],v[0]));
   tgReal const theta(asin(radius/d));
   tgReal const beta1(alpha + theta);
   tgReal const beta2(alpha - theta);
   tgReal const h(sqrt(d * d - radius * radius));
...
tgRealMatrix tgUtil2D::perpendicularLinePts(tgRealVec const& p, tgRealVec const& center, tgReal const radius)
{
   tgRealVec v(center.relativeTo(p));
