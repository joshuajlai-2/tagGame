...
tgReal tgMath::normal(tgReal const mean, tgReal const std)
{
   ...
   static tgReal rho(0);
   static tgReal r1(0);
   // tgReal const pi(3.14159265358979323846);

   if (!validCache)
   {
      tgReal const r0(uniform01());
      r1 = uniform01();
      rho = sqrt(-tgReal(2) * log(tgReal(1) - r0));
      validCache = true;

      return rho * cos(tgReal(2) * M_PI * r1) * std + mean;
   }
   else
   {
      validCache = false;
   }
   return rho * sin(tgReal(2) * M_PI * r1) * std + mean;
}
