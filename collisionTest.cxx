...
void collideTest01()
{
   ...
   tgRealVec rpc1(c0.getPosition().relativeTo(c1.getPosition()));
   tgRealVec rpc0(c1.getPosition().relativeTo(c0.getPosition()));
   ...
   TG_ASSERT(c0p->touching(c1));

   tgRealVec v(tgUtil2D::dim);
   v[0] = -1;
   v[1] = 0;
   c0.setVelocity(v);
   v[0] = 1;
...
void collideTest02()
{
   ...
   tgRealVec center(tgUtil2D::dim);
   center.set(0);
   center[0] = 4;

   tgReal radius(2);

   tgObstacle* circle(new tgObstacleCircle(&gs));
   circle->setPosition(center);
   // TODO: more sensible constructors
   static_cast<tgObstacleCircle*>(circle)->setRadius(radius);

   tgRealVec q(circle->nearestIntersection(p, v));
   ...
   tgObstacle* side(new tgObstacleSide(&gs));
   static_cast<tgObstacleSide*>(side)->setBegin(begin);
   static_cast<tgObstacleSide*>(side)->setEnd(end);
...
void collideTest03()
{
   ...
   tgObstacleSide* side(new tgObstacleSide(&gs));
   ...
   tgCharacter* c(new tgCharacter(&gs, NULL));
   c->setMass(1);
   tgRealVec x(tgUtil2D::dim);
   x.set(5);
   c->setPosition(x);
   c->setRadius(1);
   x.set(0);
   ...
   x.scale(2);
   c->setVelocity(x);
   // TODO: print methods for tgCharacter and other objects
   gs.addCharacter(c);
   ...
   tgRealVec cp(perception.nextCollisionPoint());
