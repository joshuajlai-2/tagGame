...
tgGameState::tgGameState(tgRealVec const& worldDim) :
   frame(0),
   worldDim(worldDim),
   timer(NULL),
   lastTagTime(-1)
...
void tgGameState::setTagged()
{
   lastTagTime = timer->gameTime();
}

int tgGameState::getCharacterIndex(int i) const
{  // TODO: horribly inefficient
   for (unsigned j(0); j < characterIndices.size(); j++)
   {
      if (i == characterIndices[j]) { return j; }
   }
   tgUtil::error(string("index: ") + tgUtil::itos(i) + string(" not found"));
   return nobody;
}

int tgGameState::getObstacleIndex(int i) const
{
  // TODO: horribly inefficient
   for (unsigned j(0); j < obstacleIndices.size(); j++)
   {
      if (i == obstacleIndices[j]) { return j; }
   }
   tgUtil::error(string("index: ") + tgUtil::itos(i) + string(" not found"));
   return nobody;
}

bool tgGameState::isCollidingTagged(tgCharacter const& who) const
{
   int const whoCharacterIndex(getCharacterIndex(who.getGSIndex()));

   int const colliderObstacleIndex(colliders[whoCharacterIndex]);

   if (colliderObstacleIndex < 0) { return false; }

   // TODO: remember which obstacles are characters and replace with static_cast
   tgCharacter* c(dynamic_cast<tgCharacter*>(objects[obstacleIndices[colliderObstacleIndex]]));

   if (!c) { return false; }

   return c->getIsTagged();
}

bool tgGameState::isCollidingTagged(int const who) const
{
   int const i(colliders[who]);
   if (i < 0) { return false; }

   // TODO: remember which obstacles are characters and replace with static_cast
   tgCharacter* c(dynamic_cast<tgCharacter*>(objects[obstacleIndices[i]]));
...
int tgGameState::addCharacter(tgCharacter* c)
{
   int const n(addObstacle(c));

   characterIndices.push_back(n);
   colliders.resize(characterIndices.size());
   
   return n;
}

int tgGameState::addObstacle(tgObstacle* o)
{
   int const n(objects.size());
   o->setGSIndex(n);

   obstacleIndices.push_back(n);
   objects.push_back(o);
   
   return n;
}

int tgGameState::calcColliders()
{
   int colliderCount(0);

   fill(colliders.begin(), colliders.end(), -1);

   // TODO: with some care, second loop could start from i
   // characters are the only ones who can move in the tag game
   for (unsigned i(0); i < characterIndices.size(); i++)
   {
      for (unsigned j(0); j < obstacleIndices.size(); j++)
      {
         if (characterIndices[i] == obstacleIndices[j]) { continue; }

         // Colliding is reflexive, i.e. if A is colliding with B, then B is also
         // colliding with A.  So there's no need to compute it twice.  TODO: fix.
         if (static_cast<tgObstacle*>(objects[characterIndices[i]])->colliding(*static_cast<tgObstacle*>(objects[obstacleIndices[j]])))
         {
            colliderCount++;
            colliders[i] = j;
         }
